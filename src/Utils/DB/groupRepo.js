...
const group_all = (callback) => {
    var t0 = performance.now();
    db.conn.transaction((tx) => {
        tx.executeSql("select * from group_chat;", [], (tx, results) => {
            let value = []
...
const group_insert = (data, callback) => {
    db.conn.transaction((tx) => {
        tx.executeSql("INSERT INTO group_chat ('group_id', 'data', 'create', 'update') VALUES (?, ?, ?, ?);", [data.group_id, JSON.stringify(data.value), currentTimestamp(),currentTimestamp()], (tx, results) => {
...
const group_update = (data /* { group_id:xxx,value:yyy } */, callback) => {

    var t0 = performance.now();
    group_get(data.group_id, v=>{
        // console.log(v)
        if(v.status){
            if(v.value.length < 1){
                // insert 
                ...
                if(JSON.stringify(data.group_id) === v.value[0].data){
                    ...
                    // update 
                    db.conn.transaction((tx) => {
                        tx.executeSql("UPDATE group_chat set 'data'=?, 'update'=? WHERE group_id=?;", [JSON.stringify(data.value), currentTimestamp(), data.group_id], (tx, results) => {
...
const group_deleteAll = (callback) => {
    db.conn.transaction((tx) => {
        tx.executeSql("DELETE from group_chat", [], (tx, results) => {
            callback({'status':true, 'results':results})
        })
    })
}
